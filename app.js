const APP_VERSION = '1.0.5';  // Atualize aqui a cada nova versão
...
function pageLogin(){if(IS_ADMIN_PAGE)return pageLoginAdmin();
  return`<div class="center-page">
    ...
    <div style="position:fixed;bottom:16px;left:0;right:0;text-align:center;font-size:11px;color:var(--muted);letter-spacing:.02em">© Desenvolvido por <strong style="color:var(--accent)">Kelvin Santos</strong> &nbsp;·&nbsp; <span style="opacity:.6">v${APP_VERSION}</span></div>
  </div>`;}
...
function doLogin(){
  ...
  if(c){
    S.session={isAdmin:false,clientId:c.id};
    sessionStorage.setItem('sn:session',JSON.stringify(S.session));
    // Salva versão e último acesso do cliente
    S.clients=S.clients.map(x=>x.id===c.id?{...x,appVersion:APP_VERSION,lastLogin:new Date().toISOString()}:x);
    saveClients();
    S.page=c.mustChangePwd?'changepwd':'client';render();return;
  }
...
function admClients(){
  ...
  const cards=filtered.map(c=>{
    ...
    return`<div class="cl-card" onclick="openClientDetail('${c.id}')"><div class="cl-card-header"><div class="cl-avatar">${initials||'?'}</div><div><div class="cl-name">${esc(c.razaoSocial)}</div><div class="cl-cnpj">${esc(c.cnpj)}</div><div style="font-size:11px;color:var(--accent);margin-top:2px">Login: <strong>${esc(c.username)}</strong></div>
        <div style="margin-top:4px;display:flex;align-items:center;gap:6px">
          ${c.appVersion?`<span style="font-size:10px;padding:1px 7px;border-radius:20px;font-weight:700;background:${c.appVersion===APP_VERSION?'rgba(16,185,129,.12)':'rgba(249,115,22,.12)'};color:${c.appVersion===APP_VERSION?'#10b981':'#f97316'};border:1px solid ${c.appVersion===APP_VERSION?'rgba(16,185,129,.3)':'rgba(249,115,22,.3)'}">${c.appVersion===APP_VERSION?'✓':'⚠️'} ${c.appVersion}</span>`:'<span style="font-size:10px;color:var(--muted)">versão desconhecida</span>'}
          ${c.lastLogin?`<span style="font-size:10px;color:var(--muted)">· acesso ${fmtDate(c.lastLogin.slice(0,10))}</span>`:''}
        </div></div></div><div class="cl-stats-grid"><div class="cl-stat"><div class="cl-stat-lbl">Lançamentos</div><div class="cl-stat-val" style="color:var(--blue)">${txs.length}</div></div><div class="cl-stat"><div class="cl-stat-lbl">Saldo</div><div class="cl-stat-val" style="color:${saldo>=0?'var(--success)':'var(--danger)'}">${fmtCur(saldo)}</div></div><div class="cl-stat"><div class="cl-stat-lbl">Receitas</div><div class="cl-stat-val" style="color:var(--success)">${fmtCur(rec)}</div></div><div class="cl-stat"><div class="cl-stat-lbl">Último mov.</div><div class="cl-stat-val">${lastTx?fmtDate(lastTx.date):'—'}</div></div></div><div class="cl-card-footer">${badge}<div class="row" style="gap:6px" onclick="event.stopPropagation()"><button class="btn btn-ghost btn-sm" onclick="editClientOpen('${c.id}')">Editar</button><button class="btn btn-ghost btn-sm" onclick="resetClientPwd('${c.id}')">Reset senha</button><button class="btn btn-danger btn-sm" onclick="deleteClient('${c.id}')">Remover</button></div></div></div>`;
  }).join('');
  const outdated=S.clients.filter(c=>c.appVersion&&c.appVersion!==APP_VERSION).length;
  const neverLogged=S.clients.filter(c=>!c.appVersion).length;
  const versionBar=outdated>0?`<div style="background:rgba(249,115,22,.08);border:1px solid rgba(249,115,22,.3);border-radius:8px;padding:10px 14px;margin-bottom:12px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px"><div style="display:flex;align-items:center;gap:8px"><span style="font-size:16px">⚠️</span><span style="font-size:13px;color:#f97316;font-weight:600">${outdated} cliente${outdated>1?'s':''} com versão desatualizada</span>${neverLogged?`<span style="font-size:12px;color:var(--muted)">· ${neverLogged} nunca acessaram</span>`:''}</div><span style="font-size:11px;color:var(--muted)">Versão atual: <strong style="color:var(--accent)">v${APP_VERSION}</strong></span></div>`:neverLogged>0?`<div style="background:var(--card2);border:1px solid var(--border);border-radius:8px;padding:8px 14px;margin-bottom:12px;font-size:12px;color:var(--muted)">${neverLogged} cliente${neverLogged>1?'s':''} ainda não acessaram o app · Versão atual: <strong style="color:var(--accent)">v${APP_VERSION}</strong></div>`:'';
  return`<div class="row-spread" style="margin-bottom:16px"><div><h2 style="font-size:16px;font-weight:700">${S.clients.length} cliente${S.clients.length!==1?'s':''} cadastrado${S.clients.length!==1?'s':''}${S.clientLetter?` · <span style="color:var(--accent)">${filtered.length} com "${S.clientLetter}"</span>`:''}</h2></div><div style="display:flex;gap:8px"><button class="btn btn-ghost btn-sm" onclick="copyClientLink()" title="Copiar link do app para enviar aos clientes">🔗 Copiar link do app</button><button class="btn btn-acc" onclick="openNewClientForm()">+ Novo Cliente</button></div></div>${formHtml}${versionBar}${alphaBar}${S.clients.length===0?'<div class="empty"><div class="empty-icon">👥</div>Nenhum cliente cadastrado ainda.<br><br><button class="btn btn-acc" style="margin-top:16px" onclick="openNewClientForm()">Cadastrar primeiro cliente</button></div>':filtered.length===0?`<div class="empty" style="padding-top:40px"><div style="font-size:48px;margin-bottom:12px">🔍</div><div style="color:var(--muted)">Nenhum cliente com a letra <strong style="color:var(--accent)">${S.clientLetter}</strong></div></div>`:`<div class="cl-grid">${cards}</div>`}`;
}
function copyClientLink(){
  const url='https://sncontabilidade.github.io/SNcontabilidade/';
  navigator.clipboard.writeText(url).then(()=>{
    const btn=document.querySelector('[onclick="copyClientLink()"]');
    if(btn){const orig=btn.innerHTML;btn.innerHTML='✓ Link copiado!';btn.style.color='var(--success)';setTimeout(()=>{btn.innerHTML=orig;btn.style.color='';},2000);}
